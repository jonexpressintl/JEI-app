...
// ──────────── EDITABLE EXTRA FEES ────────────
// Pencil to edit, clean panel drops below the row. No page reload on save.
function EditableExtraFees({fees,badge,badgeBg,rowBg,fmtOrig,toIDR,onSave}){
  ...
  return(<>{fees.map((fee,i)=>{
    ...
    return(
      <div key={i}>
        {/* Display row — dims slightly when edit panel is open */}
        <div style={{...S.invLine,background:rowBg,opacity:isE?.5:1,transition:"opacity .15s"}}>
          <span style={{flex:3,display:"flex",alignItems:"center",gap:6}}>
            ...
            <span>{qty>1?`${fee.label||"Additional cost"} ×${qty}`:(fee.label||"Additional cost")}</span>
          </span>
          ...
          <button
            ...
            style={{width:28,height:28,background:isE?"var(--line)":"transparent",border:"none",color:"var(--ink-3)",cursor:"pointer",padding:0,flexShrink:0,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",transition:".15s"}}
            title={isE?"Cancel edit":"Edit this line"}
          >{isE?<X size={13}/>:<Pencil size={13}/>}</button>
        </div>

        {/* Edit panel — appears below the row */}
        {isE&&(
          <div style={{marginBottom:4,background:"var(--head)",border:"1px solid var(--navy)",borderTop:"2px solid var(--navy)",borderRadius:"0 0 10px 10px",padding:"14px 16px"}}>
          ...
          return(
            <div key={c.id}>
              <div style={{...S.invLine,background:"var(--bad-bg)",opacity:isEd?.5:1,transition:"opacity .15s"}}>
                ...
                >{isEd?<X size={13}/>:<Pencil size={13}/>}</button>
              </div>
              {isEd&&(
                <div style={{margin:"0 0 4px 0",background:"var(--head)",border:"1px solid var(--bad)",borderTop:"2px solid var(--bad)",borderRadius:"0 0 10px 10px",padding:"14px 16px"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:8,alignItems:"end",marginBottom:12}}>
                    <label>
                      <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Description</div>
                      <input style={{...S.input,margin:0,width:"100%"}} value={editCostDraft.label} onChange={e=>setEditCostDraft({...editCostDraft,label:e.target.value})}/>
                    </label>
                    <label>
                  ...
                  <div style={{display:"flex",gap:8,justifyContent:"space-between"}}>
                    <button style={{...S.secBtn,color:"var(--bad)",borderColor:"var(--bad)",fontSize:12}} onClick={()=>{setEditCostId(null);removeCost(c.id);}}><Trash2 size={13}/> Delete cost</button>
                    <div style={{display:"flex",gap:8}}>
