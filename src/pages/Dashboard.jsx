...
// ──────────── EDITABLE EXTRA FEES ────────────
// Pencil icon to edit inline. Edit mode opens a clean card below the row.
function EditableExtraFees({fees,badge,badgeBg,rowBg,fmtOrig,toIDR,onSave}){
  ...
  function startEdit(i,f){
    setEditIdx(i);
    setDraft({label:f.label||"",amount:String(f.amount||0),qty:String(f.qty||1),currency:f.currency||"USD"});
  }
  async function saveEdit(i){
    ...
    setSaving(false); setEditIdx(null);
  }
  async function del(i){
    setSaving(true);
    await onSave(fees.filter((_,j)=>j!==i));
    setSaving(false); setEditIdx(null);
  }
  ...
  return(<>{fees.map((fee,i)=>{
    ...
    return(
      <div key={i}>
        {/* Display row */}
        <div style={{...S.invLine,background:rowBg,opacity:isE?.5:1,transition:"opacity .15s"}}>
          <span style={{flex:3,display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:10,background:badgeBg,color:"#fff",borderRadius:4,padding:"1px 6px",fontWeight:700,letterSpacing:".04em",flexShrink:0}}>{badge}</span>
            <span style={{fontWeight:isE?400:500}}>{qty>1?`${fee.label||"Additional cost"} ×${qty}`:(fee.label||"Additional cost")}</span>
          </span>
          ...
          <button
            onClick={()=>isE?setEditIdx(null):startEdit(i,fee)}
            style={{width:28,height:28,background:isE?"var(--line)":"transparent",border:"none",color:isE?"var(--ink-3)":"var(--ink-3)",cursor:"pointer",padding:0,flexShrink:0,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",transition:".15s"}}
            title={isE?"Cancel edit":"Edit this line"}
          >
            {isE?<X size={13}/>:<Pencil size={13}/>}
          </button>
        </div>

        {/* Edit panel — drops in below the row */}
        {isE&&(
          <div style={{margin:"0 0 4px 0",background:"var(--head)",border:"1px solid var(--navy)",borderTop:"2px solid var(--navy)",borderRadius:"0 0 10px 10px",padding:"14px 16px"}}>
            <div style={{fontSize:11,fontWeight:700,color:"var(--navy)",letterSpacing:".06em",marginBottom:10}}>EDIT LINE</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:8,alignItems:"end",marginBottom:10}}>
              <label>
                <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Description</div>
                <input style={{...S.input,margin:0,width:"100%"}} value={draft.label} onChange={e=>setDraft({...draft,label:e.target.value})} placeholder="e.g. Handling fee"/>
              </label>
              <label>
                <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Qty</div>
                <input style={{...S.input,margin:0,width:60}} type="number" min="1" value={draft.qty} onChange={e=>setDraft({...draft,qty:e.target.value})}/>
              </label>
              <label>
                <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Unit amount</div>
                <input style={{...S.input,margin:0,width:120}} type="number" value={draft.amount} onChange={e=>setDraft({...draft,amount:e.target.value})}/>
              </label>
              <label>
                <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Currency</div>
                <select style={{...S.input,margin:0,width:80}} value={draft.currency} onChange={e=>setDraft({...draft,currency:e.target.value})}>
                  <option value="USD">USD</option><option value="SGD">SGD</option><option value="IDR">IDR</option>
                </select>
              </label>
            </div>
            {+draft.qty>1&&+draft.amount&&(
              <div style={{fontSize:12,color:"var(--ink-2)",marginBottom:10}}>
                Total: {draft.qty} × {draft.currency==="USD"?`$${(+draft.amount).toFixed(2)}`:draft.currency==="SGD"?`S$${(+draft.amount).toFixed(2)}`:`Rp ${Number(+draft.amount).toLocaleString("id-ID")}`} = <strong>{draft.currency==="USD"?`$${((+draft.qty)*(+draft.amount)).toFixed(2)}`:draft.currency==="SGD"?`S$${((+draft.qty)*(+draft.amount)).toFixed(2)}`:`Rp ${Math.round((+draft.qty)*(+draft.amount)).toLocaleString("id-ID")}`}</strong>
              </div>
            )}
            <div style={{display:"flex",gap:8,justifyContent:"space-between"}}>
              <button style={{...S.secBtn,color:"var(--bad)",borderColor:"var(--bad)",fontSize:12}} onClick={()=>del(i)} disabled={saving}>
                <Trash2 size={13}/> Delete line
              </button>
              <div style={{display:"flex",gap:8}}>
                <button style={{...S.secBtn,fontSize:12}} onClick={()=>setEditIdx(null)} disabled={saving}>Cancel</button>
                <button style={{...S.printBtn,fontSize:12}} onClick={()=>saveEdit(i)} disabled={saving}>
...
function InvoiceDoc({ctx,order,onComplete,reload}){
  ...
  return(
    <div style={S.invoice}>
      ...
      {/* Main fees (weight/seafreight/airfreight) — read-only */}
      {q.feeLines.map((l,i)=>(
      ...
      <div style={{...S.addCostBox,borderColor:"var(--bad)"}}>
        ...
        {localCosts.map((c,i)=>{
          ...
          return(
            <div key={c.id}>
              {/* Display row */}
              <div style={{...S.invLine,background:"var(--bad-bg)",opacity:isEd?.5:1,transition:"opacity .15s"}}>
                <span style={{flex:3,color:"var(--bad)",fontWeight:500}}>{c.label}</span>
                ...
                <button
                  onClick={()=>isEd?setEditCostId(null):(setEditCostId(c.id),setEditCostDraft({label:c.label,amount:String(c.amount),currency:c.currency}))}
                  style={{width:28,height:28,background:isEd?"var(--line)":"transparent",border:"none",color:"var(--ink-3)",cursor:"pointer",padding:0,flexShrink:0,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",transition:".15s"}}
                  title={isEd?"Cancel":"Edit this cost"}
                >
                  {isEd?<X size={13}/>:<Pencil size={13}/>}
                </button>
              </div>
              {/* Edit panel */}
              {isEd&&(
                <div style={{margin:"0 0 4px 0",background:"var(--head)",border:"1px solid var(--bad)",borderTop:"2px solid var(--bad)",borderRadius:"0 0 10px 10px",padding:"14px 16px"}}>
                  <div style={{fontSize:11,fontWeight:700,color:"var(--bad)",letterSpacing:".06em",marginBottom:10}}>EDIT COST</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:8,alignItems:"end",marginBottom:12}}>
                    <label>
                      <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Description</div>
                      <input style={{...S.input,margin:0,width:"100%"}} value={editCostDraft.label} onChange={e=>setEditCostDraft({...editCostDraft,label:e.target.value})} placeholder="Cost description"/>
                    </label>
                    <label>
                      <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Amount</div>
                      <input style={{...S.input,margin:0,width:120}} type="number" value={editCostDraft.amount} onChange={e=>setEditCostDraft({...editCostDraft,amount:e.target.value})}/>
                    </label>
                    <label>
                      <div style={{fontSize:11,color:"var(--ink-3)",fontWeight:600,marginBottom:4}}>Currency</div>
                      <select style={{...S.input,margin:0,width:80}} value={editCostDraft.currency} onChange={e=>setEditCostDraft({...editCostDraft,currency:e.target.value})}>
                        <option value="USD">USD</option><option value="SGD">SGD</option><option value="IDR">IDR</option>
                      </select>
                    </label>
                  </div>
                  <div style={{display:"flex",gap:8,justifyContent:"space-between"}}>
                    <button style={{...S.secBtn,color:"var(--bad)",borderColor:"var(--bad)",fontSize:12}} onClick={()=>{setEditCostId(null);removeCost(c.id);}}>
                      <Trash2 size={13}/> Delete cost
                    </button>
                    <div style={{display:"flex",gap:8}}>
                      <button style={{...S.secBtn,fontSize:12}} onClick={()=>setEditCostId(null)}>Cancel</button>
                      <button style={{...S.printBtn,background:"var(--bad)",fontSize:12}} onClick={async()=>{
                        ...
                        setEditCostId(null);
                      }}><Check size={13}/> Save changes</button>
                    </div>
                  </div>
                </div>
              )}
