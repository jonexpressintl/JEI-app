...
const PW = 180;
const PAGE_H = 297;
const FOOTER_SPACE = 72; // space needed for bank details block
...
// Returns currentY if there's room, otherwise adds a page and returns 20
function ensureSpace(doc, currentY, needed = 10) {
  if (currentY + needed > PAGE_H - 12) {
    doc.addPage();
    return 20;
  }
  return currentY;
...
function drawBankDetails(doc, startY) {
  let py = ensureSpace(doc, startY, FOOTER_SPACE);
...
export function generateInvoicePDF(order, customer, shipment, courier, liveFx) {
  ...
  // Build fee lines identical to Dashboard quote()
  ...
  if (feeMode === "air_air" || (feeMode === "air_sea" && order.air_sea_option !== "breakdown")) {
    ...
    const seaCBMTotalP = (+order.fee_2 || 0) * cbmB;
    const cur1 = order.fee_1_cur || "USD"; const cur2 = order.fee_2_cur || "IDR";
    const sym1 = cur1 === "USD" ? "$" : cur1 === "SGD" ? "S$" : "Rp";
    const sym2 = cur2 === "USD" ? "$" : cur2 === "SGD" ? "S$" : "Rp";
  ...
  // Fee table — autoTable handles page breaks automatically
...
export function generateQuotationPDF({ customerName, packages, divisor, courierName, ratePerKg, priceCurrency }) {
  ...
  pkgList.forEach((p, i) => {
    ...
    totalRaw += ch.raw; totalActual += +p.weight; totalVol += ch.vol;
    body.push([`Package ${i + 1}`, `${(+p.weight).toFixed(1)} kg`, `${(+p.l).toFixed(0)} × ${(+p.w).toFixed(0)} × ${(+p.h).toFixed(0)} cm`, `${ch.vol.toFixed(1)} kg`, `${ch.raw.toFixed(1)} kg`, ch.basis]);
  });
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [{ content: "Total Chargeable Weight", styles: { fontStyle: "bold", fillColor: [220, 245, 240] } }, { content: `${totalCharged.toFixed(1)} kg` + (minApplied ? " (3kg min)" : ""), styles: { fontStyle: "bold", fillColor: [220, 245, 240] } }],
      ["", ""],
      [{ content: "Rate per kg", styles: { fontStyle: "bold" } }, fmtAmt(ratePerKg, cur)],
      [{ content: "Estimated Total", styles: { fontStyle: "bold", fontSize: 11, fillColor: BG } }, { content: fmtAmt(total, cur), styles: { fontStyle: "bold", fontSize: 11, fillColor: BG } }],
    ],
